...
''' Security Options '''
help_opt = {
    'sec-action': {'{h}--sec-action <string>{e}': {'description': 'akce - export|scan|spider|start|stop', 'commands': ('sec-zap')}},
    ...
    'sec-method': {'{h}[--sec-method <string>]{e}': {'description': 'HTTP metoda, default GET, podporováno pro akci scan', 'commands': ('sec-zap')}},
    'sec-output': {'{h}[--sec-output <filename>]{e}': {'description': 'název výstupního souboru, podporováno pro akci export', 'commands': ('sec-zap')}},
    'sec-params': {'{h}[--sec-params <dict>]{e}': {'description': 'parametry requestu key1:val1|key2:val2, podporováno pro akce scan|spider', 'commands': ('sec-zap')}},
    'sec-path': {'{h}[--sec-path <path>]{e}': {'description': 'cesta k proxy, default zap.sh, podporováno pro akci start', 'commands': ('sec-zap')}},
    ...
    # standalone with option profile security
    'action': {'{h}--action <string>{e}': {'description': 'akce - export|scan|spider|start|stop', 'commands': ('zap')}},
    ...
    'method': {'{h}[--method <string>]{e}': {'description': 'HTTP metoda, default GET, podporováno pro akci scan', 'commands': ('zap')}},
    'output': {'{h}[--output <filename>]{e}': {'description': 'název výstupního souboru, podporováno pro akci export', 'commands': ('zap')}},
    'params': {'{h}[--params <dict>]{e}': {'description': 'parametry requestu key1:val1|key2:val2, podporováno pro akce scan|spider', 'commands': ('zap')}},
    'path': {'{h}[--path <path>]{e}': {'description': 'cesta k proxy, default zap.sh, podporováno pro akci start', 'commands': ('zap')}},
